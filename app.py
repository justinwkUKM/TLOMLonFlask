...
def index_api():
    ...
    df_forecast14 = pd.DataFrame(data=future_sales_list_14, index = days, columns=['Sales_Time'])
    forecast_sarimax_exog = model.predict(447, 447+13, exog = df_forecast14[['Sales_Time']], typ='levels').rename("SARIMAX ForeCasts")
...
def predict():
    ...
    if request.method == 'POST':
        ...
        df_forecast7 = pd.DataFrame(data=future_sales_list_7, index = days, columns=['Sales_Time'])
        
        if data != 0:
            forecast_sarimax_exog = model.predict(447, 447+6, exog = df_forecast7[['Sales_Time']], typ='levels').rename("SARIMAX ForeCasts")
